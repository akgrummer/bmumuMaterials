...
TH1F* BkgEffHist(TH1F* myHist, VariableInfo myVar = Bisolation){
    TH1F* myHistRet(NULL);
    myHistRet = (TH1F*)myHist->Clone("myHistRet");
    Int_t NumberOfBinsROC = myVar.nbins;
    vector<Double_t> BkgTestEff;
    vector<Double_t> BkgTestEff_err;
    for(int i = 0; i<NumberOfBinsROC; i++){
  // cout<<i<<endl;
      BkgTestEff.push_back((myHist->Integral(NumberOfBinsROC-i, NumberOfBinsROC))/(myHist->Integral(1, NumberOfBinsROC)));
    }
    for(int i = 0; i<NumberOfBinsROC; i++){
      BkgTestEff_err.push_back(sqrt((BkgTestEff[i]*(1-BkgTestEff[i]))/(myHist->Integral(0, NumberOfBinsROC) ) ) );
    }
    for (int i = 0; i<NumberOfBinsROC; i++){
        myHistRet->SetBinContent(NumberOfBinsROC-i, BkgTestEff[i]);
        myHistRet->SetBinError(NumberOfBinsROC-i, BkgTestEff_err[i]);
    }
    return myHistRet;
}

TH1F* SigEffHist(TH1F* myHist, TH1F* myHist_w2, VariableInfo myVar = Bisolation){
    int NumberOfBinsROC = myVar.nbins;
    vector<Double_t> SigTestEff;
    vector<Double_t> SigTestEff_err;
    for(int i = 0; i<NumberOfBinsROC; i++){
        SigTestEff.push_back((myHist->Integral(NumberOfBinsROC-i, NumberOfBinsROC))/(myHist->Integral(1, NumberOfBinsROC)));
    }
    vector<Double_t> wp;
    vector<Double_t> w2p;
    vector<Double_t> wm;
    ...
    vector<Double_t> w2;
    for(int i = 0; i<NumberOfBinsROC; i++){
        wp.push_back(myHist->Integral(NumberOfBinsROC-i, NumberOfBinsROC));
        w2p.push_back(myHist_w2->Integral(NumberOfBinsROC-i, NumberOfBinsROC));
        wm.push_back(myHist->Integral(0, NumberOfBinsROC-i));
        w2m.push_back(myHist_w2->Integral(0, NumberOfBinsROC-i));
        w2.push_back((myHist->Integral(0, NumberOfBinsROC))*(myHist->Integral(0, NumberOfBinsROC)));
    }
    for(int i = 0; i<NumberOfBinsROC; i++){
    ...
    for (int i = 0; i<NumberOfBinsROC; i++){
        myHist->SetBinContent(NumberOfBinsROC-i, SigTestEff[i]);
        myHist->SetBinError(NumberOfBinsROC-i, SigTestEff_err[i]);
    }
    return myHist;
}


TH1F* BkgEffHist_Reverse(TH1F* myHist, VariableInfo myVar = Bisolation){
    TH1F* myHistRet(NULL);
    myHistRet = (TH1F*)myHist->Clone("myHistRet");
    Int_t NumberOfBinsROC = myVar.nbins;
    vector<Double_t> BkgTestEff;
    vector<Double_t> BkgTestEff_err;
    for(int i = 1; i<NumberOfBinsROC+1; i++){
  // cout<<i<<endl;
        BkgTestEff.push_back((myHist->Integral(0, i))/(myHist->Integral(0, NumberOfBinsROC)));
    }
    for(int i = 0; i<NumberOfBinsROC; i++){
        BkgTestEff_err.push_back(sqrt((BkgTestEff[i]*(1-BkgTestEff[i]))/(myHist->Integral(0, NumberOfBinsROC) ) ) );
    }
    for (int i = 0; i<NumberOfBinsROC; i++){
        //need the "+1" to compensate for the underflow and overflow properly.
        myHistRet->SetBinContent(i+1, BkgTestEff[i]);
        myHistRet->SetBinError(i+1, BkgTestEff_err[i]);
    }
    return myHistRet;
}

TH1F* SigEffHist_Reverse(TH1F* myHist, TH1F* myHist_w2, VariableInfo myVar = Bisolation){
    int NumberOfBinsROC = myVar.nbins;
    vector<Double_t> SigTestEff;
    vector<Double_t> SigTestEff_err;
    for(int i = 1; i<NumberOfBinsROC+1; i++){
        SigTestEff.push_back((myHist->Integral(0, i))/(myHist->Integral(0, NumberOfBinsROC)));
    }
    vector<Double_t> wp;
    vector<Double_t> w2p;
    vector<Double_t> wm;
    ...
    vector<Double_t> w2;
    for(int i = 1; i<NumberOfBinsROC+1; i++){
        wp.push_back(myHist->Integral(0, i));
        w2p.push_back(myHist_w2->Integral(0, i));
        wm.push_back(myHist->Integral(i, NumberOfBinsROC));
        w2m.push_back(myHist_w2->Integral(i, NumberOfBinsROC));
        w2.push_back((myHist->Integral(0, NumberOfBinsROC))*(myHist->Integral(0, NumberOfBinsROC)));
    }
    for(int i = 0; i<NumberOfBinsROC; i++){
        SigTestEff_err.push_back(sqrt(w2p[i]*wm[i]*wm[i]+w2m[i]*wp[i]*wp[i])/w2[i] ) ;
    ...
    for (int i = 0; i<NumberOfBinsROC; i++){
        //need the "+1" to compensate for the underflow and overflow properly.
        myHist->SetBinContent(i+1, SigTestEff[i]);
        myHist->SetBinError(i+1, SigTestEff_err[i]);
    }
    return myHist;
}
...
TH1F* RetrieveHist(TString tmpYear = "",  TString DataType = "", 
                    ...
                    TString FileDate = "", TString Pileup= "", TString theVar = "") {
...
if (DataType.EqualTo("data")) {
    if (tmpYear.EqualTo("2016")){
        ...
        myCut = CutAdd;
        myCut_w2= CutAdd*RunNumberCut*"PVWeights*Muon1_reco_eff_sf*Muon2_reco_eff_sf"*"PVWeights*Muon1_reco_eff_sf*Muon2_reco_eff_sf";
...
TString weightsName = myVar.name;
weightsName+="Weights";
if (theVar.EqualTo("DOCA")){
    if (DataType.EqualTo("data")) {
        myHist = BkgEffHist(myHist, myVar);
    }else if (DataType.EqualTo("mc")) {
        myHist_w2 = getHistoFromTree(myTree, myVar, weightsName, myCut_w2);
        myHist = SigEffHist(myHist, myHist_w2, myVar);
    }
}else{
    if (DataType.EqualTo("data")) {
        myHist = BkgEffHist_Reverse(myHist, myVar);
    }else if (DataType.EqualTo("mc")) {
        myHist_w2 = getHistoFromTree(myTree, myVar, weightsName, myCut_w2);
        myHist = SigEffHist_Reverse(myHist, myHist_w2, myVar);
    }
}
...
myHist->GetYaxis()->SetTitle("Efficiency");
return myHist;
}
...
if (DataType.EqualTo("data")) {
        ...
        myCut_w2= CutRun1*"PVWeights"*"PVWeights";
    }
...
if (!Run1set){
ColorIndex = 0;
h_lowlow = RetrieveHist("2018", DataType, myVar, ColorIndex, FileDate, "lowlow", theVar);
ColorIndex = 1;
h_low = RetrieveHist("2018", DataType, myVar, ColorIndex, FileDate, "low", theVar);
ColorIndex = 2;
h_medium = RetrieveHist("2017", DataType, myVar, ColorIndex, FileDate, "medium", theVar);
ColorIndex = 3;
h_high = RetrieveHist("2017", DataType, myVar, ColorIndex, FileDate, "high", theVar);
}else{
h_lowlow = RetrieveHistRun1(DataType, myVar, ColorIndex, FileDate, "lowlow", theVar);
ColorIndex = 1;
h_low = RetrieveHistRun1(DataType, myVar, ColorIndex, FileDate, "low", theVar);
ColorIndex = 2;
h_medium = RetrieveHistRun1(DataType, myVar, ColorIndex, FileDate, "medium", theVar);  
}
...
TCanvas* c2 = new TCanvas("c2", "Var Comps",0.,0., 800, 600);
    ...
    if(DataType.EqualTo("data")){
        ...
        //WARNING SetLimits is dangerous (changes axis - not histogram!!) Must be removed from all codes!
        // h_lowlow->GetXaxis()->SetLimits(xmin, xmax);// along X
        // h_low->GetXaxis()->SetLimits(xmin, xmax);// along X
        // h_medium->GetXaxis()->SetLimits(xmin, xmax);// along X
        // if (!Run1set) h_high->GetXaxis()->SetLimits(xmin, xmax);// along X        
...
if (DataType.EqualTo("data")) {
    if (theVar.EqualTo("nCloseTrks")){
        atlasLabel(0.62, 0.46, false, true, 1, true, false, true);
        leg2 = new TLegend(0.61, 0.22, 0.81, 0.45,NULL,"brNDC");
        }
    else{
    ...
    if (theVar.EqualTo("nCloseTrks")){
        ...
        atlasLabel(0.23, 0.46, false, true, 1, true, false, true);
        leg2 = new TLegend(0.22, 0.22, 0.42, 0.45,NULL,"brNDC");
    }
}
...
// c2->SaveAs(OutdirName + "Figures/VarComps2019Aug13_PU_Eff/"+varDir+"/"+typeDir+"/VarComp_"+Tag+".pdf");
// c2->SaveAs(OutdirName + "Figures/VarComps2019Sep3_PU_Eff/"+varDir+"/"+typeDir+"/VarComp_"+Tag+".pdf");
c2->SaveAs(OutdirName + "Figures/VarComps2019Nov21_PU_Eff_2018/"+varDir+"/"+typeDir+"/VarComp_"+Tag+".pdf");
delete c2;
delete h_lowlow;
delete h_low;
delete h_medium;
...
void OtherIso_MultiVar_TTVA_v6_PU_eff_2() {
