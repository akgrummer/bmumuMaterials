...
TH1F* BkgEffHist_Reverse(TH1F* myHist, VariableInfo myVar = Bisolation){
  ...
  // cout<<i<<endl;
    ...
    for (int i = 0; i<NumberOfBinsROC; i++){
        myHistRet->SetBinContent(i, BkgTestEff[i]);
        myHistRet->SetBinError(i, BkgTestEff_err[i]);
    }
    return myHistRet;
}
...
TH1F* SigEffHist_Reverse(TH1F* myHist, TH1F* myHist_w2, VariableInfo myVar = Bisolation){
    ...
    for (int i = 0; i<NumberOfBinsROC; i++){
        myHist->SetBinContent(i, SigTestEff[i]);
        myHist->SetBinError(i, SigTestEff_err[i]);
    }
    return myHist;
}
...
void OtherIso_MultiVar_TTVA_v5_PU_eff() {
