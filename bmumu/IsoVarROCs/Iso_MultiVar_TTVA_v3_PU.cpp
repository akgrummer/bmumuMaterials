...
myCut = myCut*("PRIVX_mu >= 21. && PRIVX_mu <= 32.");
}else if(Pileup.EqualTo("medium")) {
myCut = myCut*("PRIVX_mu >= 34. && PRIVX_mu <= 43.");
}else if(Pileup.EqualTo("high")) {
myCut = myCut*("PRIVX_mu >= 44. && PRIVX_mu <= 55.");
} 
}else if (tmpYear.Contains("2017")){
    lowPU_1 = 22.493;
    lowPU_2 = 29.0;
    mediumPU_1 = 36.352;
    mediumPU_2 = 43.;
    ...
    highPU_2 = 55.;
if (Pileup.EqualTo("low")) {
myCut = myCut*("PRIVX_mu >= 22.493 && PRIVX_mu <= 29.0");
}else if(Pileup.EqualTo("medium")) {
myCut = myCut*("PRIVX_mu >= 36.352 && PRIVX_mu <= 43.");
}else if(Pileup.EqualTo("high")) {
...
if (DataType.EqualTo("data")) {
    ...
    leg2->SetHeader(theYear + theRunNumber+theIDHits+myPileup);
}
else if (DataType.EqualTo("mc")) {
    atlasLabel(0.5, 0.83, false, true, 1, true, false, true);
    leg2 = new TLegend(0.22, 0.67, 0.42, 0.9,NULL,"brNDC");
    leg2->SetHeader(mcName + theRunNumber+theIDHits+myPileup);
}
cout<<"DEBUG: 5"<<endl;
...
c2->SaveAs(OutdirName + "Figures/VarComps2019July17_PU/VarComp_"+Tag+".pdf");
delete c2;
delete hRun1;
