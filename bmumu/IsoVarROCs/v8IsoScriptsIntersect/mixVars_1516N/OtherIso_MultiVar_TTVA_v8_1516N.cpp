...
if (DataType.EqualTo("data")) {
    if (tmpYear.Contains("2016")){
        // ifilename = DirName + tmpYear+"Allcommon/LASTskim_"+tmpYear+"_All_2019Jun5.root";
        if ( !DirName.Contains("2020")){tmpYear = "2016";}
        ifilename = DirName + tmpYear+"Allcommon/LASTskim_"+tmpYear+"_All_"+FileDate+".root";
    }
    else{
        // ifilename = DirName + tmpYear+"Allcommon/LASTskim_"+tmpYear+"_All_2019Jun3.root";
        ifilename = DirName + tmpYear+"Allcommon/LASTskim_"+tmpYear+"_All_"+FileDate+".root";
    }
        myCut = CutAdd;
    }
    ...
    if (tmpYear.Contains("2016")){
        // ifilename = DirName + tmp_mcName+"Allcommon/LASTskim_"+tmp_mcName+"_All_2019Jun5.root";
        ifilename = DirName + tmp_mcName+"Allcommon/LASTskim_"+tmp_mcName+"_All_"+FileDate+".root";
    }else{
        // ifilename = DirName + tmp_mcName+"Allcommon/LASTskim_"+tmp_mcName+"_All_2019Jun3.root";
        ifilename = DirName + tmp_mcName+"Allcommon/LASTskim_"+tmp_mcName+"_All_"+FileDate+".root";
    }
...
ColorIndex = 2;
h_1516 = RetrieveHist(DirName, theYear, DataType, closeTrkNtrksChi2_LooSiHi1Pt05_corc2cnt, ColorIndex, FileDate, myPileup);
ColorIndex = 1;
...
ColorIndex = 2;
h_1516 = RetrieveHist(DirName, theYear, DataType, closeTrkDOCA_LooSiHi1Pt05_corc2cnt, ColorIndex, FileDate, myPileup);
ColorIndex = 1;
h_DefaultNEW = RetrieveHist(DirName, theYear, DataType, closeTrkDOCA_comp, ColorIndex, FileDate, myPileup);
DirName = "/eos/home-a/agrummer/BsmumuData/FinalSkim/v6/";
ColorIndex = 3;
h_DefaultOLD = RetrieveHist(DirName, theYear, DataType, 
    closeTrkDOCA_comp, ColorIndex, "2019Dec5", myPileup);
}
//
// ColorIndex = 4;
...
TCanvas* c2 = new TCanvas("c2", "Var Comps",0.,0., 800, 600);
    ...
    h_DefaultOLD->GetYaxis()->SetNdivisions(4,5,0); //4,5,0
    h_DefaultOLD->GetXaxis()->SetLabelOffset(0.028);
    h_DefaultOLD->GetYaxis()->SetTitleSize(0.08);
    h_DefaultOLD->GetYaxis()->SetTitleOffset(0.9);
    h_DefaultOLD->GetYaxis()->SetLabelSize(0.08);
    ...
    if(DataType.EqualTo("data")){
        ...
        // h_DefaultNEW
        // h_DefaultOLD
            h_DefaultOLD->GetXaxis()->SetRangeUser(xmin, xmax);// along X
            h_DefaultOLD->SetMaximum(ymax);   // along          
            h_DefaultOLD->SetMinimum(ymin);  //   Y  
        h_DefaultOLD->Draw("e0p");
        h_DefaultNEW->SetMarkerStyle(23);
        h_DefaultNEW->Draw("e0psame");
        // h_1516->Draw("e0psame");
    }
    else if(DataType.EqualTo("mc")){
        if(theVar.EqualTo("DOCA")){
        ...
        h_DefaultNEW->SetLineStyle(2);
        h_DefaultNEW->Draw("hist e0 same");
...
if(DataType.EqualTo("data")){
    if (theVar.EqualTo("DOCA")){
        ...
        leg2->AddEntry(h_DefaultNEW, "ACK, new nTuple, derivation","pe");
        // leg2->AddEntry(h_DefaultOLD, "As in 2015+2016 old nTuple, default","pe");
        leg2->AddEntry(h_DefaultOLD, "ACK, old nTuple, derivation","pe");
    }else{
        // leg2->AddEntry(h_1516, "As in 2015+2016 new nTuple, Derivation","pe");
        leg2->AddEntry(h_DefaultNEW, "ACK, new nTuple, derivation","pe");
        leg2->AddEntry(h_DefaultOLD, "ACK, old nTuple, derivation","pe");
    ...
    if (theVar.EqualTo("DOCA")){
        ...
        leg2->AddEntry(h_DefaultNEW, "ACK, new nTuple, derivation","l");
        leg2->AddEntry(h_DefaultOLD, "ACK, old nTuple, derivation","l");
    }else{
        // leg2->AddEntry(h_1516, "As in 2015+2016 new nTuple, Derivation","l");
        leg2->AddEntry(h_DefaultNEW, "ACK, new nTuple, derivation","l");
        leg2->AddEntry(h_DefaultOLD, "ACK, old nTuple, derivation","l");
...
leg2->Draw();
    ...
    if (drawResiduals) {
        pad2->cd();
        hres = (TH1F*)h_DefaultOLD->Clone("hres");
        hres->Sumw2();
        hres->Divide(h_DefaultOLD,h_DefaultNEW,1.0,1.0);
...
c2->SaveAs(OutdirName + "Figures/VarComps2020Mar6_VarBinned_v8_mixVars_1516Ni/"+varDir+"/"+typeDir+"/VarComp_"+Tag+".pdf");
delete c2;
// delete h_Default;
...
void OtherIso_MultiVar_TTVA_v8_1516N() {
