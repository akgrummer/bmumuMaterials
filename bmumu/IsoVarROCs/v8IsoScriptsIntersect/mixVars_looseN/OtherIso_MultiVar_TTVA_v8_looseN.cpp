...
TCanvas* c2 = new TCanvas("c2", "Var Comps",0.,0., 800, 600);
    ...
    hdata_TTVA_looseOLD->GetYaxis()->SetNdivisions(4,5,0); //4,5,0
    hdata_TTVA_looseOLD->GetXaxis()->SetLabelOffset(0.028);
    hdata_TTVA_looseOLD->GetYaxis()->SetTitleSize(0.08);
    hdata_TTVA_looseOLD->GetYaxis()->SetTitleOffset(0.9);
    hdata_TTVA_looseOLD->GetYaxis()->SetLabelSize(0.08);
...
// hdata_TTVA_looseNEW
// hdata_TTVA_looseOLD
            hdata_TTVA_looseOLD->GetXaxis()->SetRangeUser(xmin, xmax);// along X
            hdata_TTVA_looseOLD->SetMaximum(ymax);   // along          
            hdata_TTVA_looseOLD->SetMinimum(ymin);  //   Y  
        hdata_TTVA_looseOLD->Draw("e0p");
        hdata_TTVA_looseNEW->SetMarkerStyle(23);
        hdata_TTVA_looseNEW->Draw("e0psame");
        ...
        hdata_TTVA_looseNEW->SetLineStyle(2);
        hdata_TTVA_looseNEW->Draw("hist e0 same");
...
if(DataType.EqualTo("data")){
    if (theVar.EqualTo("DOCA")){
        ...
        leg2->AddEntry(hdata_TTVA_looseNEW, "BGM, new nTuple, from trks","pe");
        // leg2->AddEntry(hdata_TTVA_looseOLD, "Loose TVA old nTuple, from Trks","pe");
        leg2->AddEntry(hdata_TTVA_looseOLD, "BGM, old nTuple, from trks","pe");
    }else{
        // leg2->AddEntry(h_looseTrkPerigee, "Loose TVA new nTuple, Derivation","pe");
        // leg2->AddEntry(h_looseApprox, "Loose TVA Approx., new nTuple, Derivation","pe");
        leg2->AddEntry(hdata_TTVA_looseNEW, "BGM, new nTuple, from trks","pe");
    ...
    if (theVar.EqualTo("DOCA")){
        // leg2->AddEntry(h_looseTrkPerigee, "Loose TVA new nTuple, Derivation","l");
        // leg2->AddEntry(h_looseApprox, "Loose TVA Approx., new nTuple, Derivation","l");
        leg2->AddEntry(hdata_TTVA_looseNEW, "BGM, new nTuple, from trks","l");
        leg2->AddEntry(hdata_TTVA_looseOLD, "BGM, old nTuple, from trks","l");
    }else{
        // leg2->AddEntry(h_looseTrkPerigee, "Loose TVA new nTuple, Derivation","l");
        // leg2->AddEntry(h_looseApprox, "Loose TVA Approx., new nTuple, Derivation","l");
        leg2->AddEntry(hdata_TTVA_looseNEW, "BGM, new nTuple, from trks","l");
...
leg2->Draw();
    ...
    if (drawResiduals) {
        pad2->cd();
        hres = (TH1F*)hdata_TTVA_looseOLD->Clone("hres");
        hres->Sumw2();
        hres->Divide(hdata_TTVA_looseOLD,hdata_TTVA_looseNEW,1.0,1.0);
...
void OtherIso_MultiVar_TTVA_v8_looseN() {
