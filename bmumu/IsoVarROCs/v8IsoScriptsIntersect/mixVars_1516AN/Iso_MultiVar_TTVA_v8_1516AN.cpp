...
TCanvas* c2 = new TCanvas("c2", "Var Comps",0.,0., 800, 600);
    ...
    h_all_c5OLD->GetYaxis()->SetNdivisions(4,5,0); //4,5,0
    h_all_c5OLD->GetXaxis()->SetLabelOffset(0.028);
    h_all_c5OLD->GetYaxis()->SetTitleSize(0.08);
    h_all_c5OLD->GetYaxis()->SetTitleOffset(0.9);
    h_all_c5OLD->GetYaxis()->SetLabelSize(0.08);
...
// h_all_c5OLD
        ...
        h_all_c5OLD->SetMaximum(ymax);   // along          
        h_all_c5OLD->SetMinimum(ymin);  //   Y  
        h_all_c5OLD->SetMaximum(5.);
        h_all_c5OLD->Draw("hist e0");
        h_all_c5NEW->SetLineStyle(2);
...
leg2->AddEntry(h_all_c5NEW, "BDI, new nTuple, from trks","pe");
// leg2->AddEntry(h_all_c5OLD, "All tracks, chi2<5, old nTuple","pe");
leg2->AddEntry(h_all_c5OLD, "BDI, old nTuple, from trks","pe");
}else if(DataType.EqualTo("mc")){
...
leg2->Draw();
    ...
    if (drawResiduals) {
        pad2->cd();
        hres = (TH1F*)h_all_c5OLD->Clone("hres");
        hres->Sumw2();
        hres->Divide(h_all_c5OLD,h_all_c5NEW,1.0,1.0);
...
c2->SaveAs(OutdirName + "Figures/VarComps2020Mar6_VarBinned_v8_mixVars_1516ANi/"+varDir+"/"+typeDir+"/VarComp_"+Tag+".pdf");
delete c2;
delete h_asRun1;
delete h_1516;
delete h_DefaultNEW;
...
void Iso_MultiVar_TTVA_v8_1516AN(){
    TString FileDate = "";
    // FileDate = "2019July30";
    // FileDate = "2019Nov21";
    // FileDate = "2019Dec5";
