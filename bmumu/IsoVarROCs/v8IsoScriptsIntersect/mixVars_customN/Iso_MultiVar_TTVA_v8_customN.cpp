...
if (DataType.EqualTo("data")) {
        ...
        ifilename = DirName + tmpYear+"Allcommon/LASTskim_"+tmpYear+"_All_"+FileDate+".root";
        cout<< "ifilename "<< ifilename<<endl;
        myCut = CutAdd;
    }
else if (DataType.EqualTo("mc")) { 
        ifilename = DirName + tmp_mcName+"Allcommon/LASTskim_"+tmp_mcName+"_All_"+FileDate+".root";
        cout<< "ifilename "<< ifilename<<endl;
...
TCanvas* c2 = new TCanvas("c2", "Var Comps",0.,0., 800, 600);
    ...
    if (drawResiduals) {
        ...
        // pad1->SetMargin(0.16,0.05,0.028,0.09);
    ...
    h_TTVA_custom_drOLD->GetYaxis()->SetNdivisions(4,5,0); //4,5,0
    h_TTVA_custom_drOLD->GetXaxis()->SetLabelOffset(0.028);
    h_TTVA_custom_drOLD->GetYaxis()->SetTitleSize(0.08);
    h_TTVA_custom_drOLD->GetYaxis()->SetTitleOffset(0.9);
    h_TTVA_custom_drOLD->GetYaxis()->SetLabelSize(0.08);
    ...
    if(DataType.EqualTo("data")){
        ...
        // h_TTVA_custom_drNEW
        // h_TTVA_custom_drOLD
        h_TTVA_custom_drOLD->SetMaximum(ymax);   // along          
        h_TTVA_custom_drOLD->SetMinimum(ymin);  //   Y  
        h_TTVA_custom_drOLD->Draw("e0p");
        h_TTVA_custom_drNEW->SetMarkerStyle(23);
        h_TTVA_custom_drNEW->Draw("e0psame");
        ...
        h_TTVA_custom_drOLD->SetMaximum(ymax);   // along          
        h_TTVA_custom_drOLD->SetMinimum(ymin);  //   Y  
        h_TTVA_custom_drOLD->SetMaximum(5.);
        h_TTVA_custom_drOLD->Draw("hist e0");
        h_TTVA_custom_drNEW->SetLineStyle(2);
        h_TTVA_custom_drNEW->Draw("hist e0 same");
...
leg2->AddEntry(h_TTVA_custom_drNEW, "BFJ, new nTuple, from trks","pe");
// leg2->AddEntry(h_TTVA_custom_drOLD, "Custom TVA old nTuple, from Trks","pe");
leg2->AddEntry(h_TTVA_custom_drOLD, "BFJ, old nTuple, from trks","pe");
}else if(DataType.EqualTo("mc")){
// if (!myPileup.EqualTo("high"))
// leg2->AddEntry(h_TTVA_custom_drNEW, "Custom TVA new nTuple, from Trks","l");
leg2->AddEntry(h_TTVA_custom_drNEW, "BFJ, new nTuple, from trks","l");
// leg2->AddEntry(h_TTVA_custom_drOLD, "Custom TVA old nTuple, from Trks","l");
leg2->AddEntry(h_TTVA_custom_drOLD, "BFJ, old nTuple, from trks","l");
// leg2->AddEntry(h_TTVA_custom_drNEW, "Custom TVA new nTuple, from Trks","l");
// leg2->AddEntry(h_TTVA_custom_drOLD, "Custom TVA old nTuple, from Trks","l");
    }
// cout<<"DEBUG: 6"<<endl;
...
leg2->Draw();
    ...
    if (drawResiduals) {
        pad2->cd();
        hres = (TH1F*)h_TTVA_custom_drOLD->Clone("hres");
        hres->Sumw2();
        hres->Divide(h_TTVA_custom_drOLD,h_TTVA_custom_drNEW,1.0,1.0);
        // hres->GetXaxis()->SetRangeUser(xmin, xmax);
        hres->SetMaximum(1.001);   // along          
        hres->SetMinimum(0.999);  //   Y 
...
c2->SaveAs(OutdirName + "Figures/VarComps2020Mar6_VarBinned_v8_mixVars_customNi/"+varDir+"/"+typeDir+"/VarComp_"+Tag+".pdf");
delete c2;
...
void Iso_MultiVar_TTVA_v8_customN(){
    TString FileDate = "";
    // FileDate = "2019July30";
    // FileDate = "2019Nov21";
    // FileDate = "2019Dec5";
