...
TCanvas* c2 = new TCanvas("c2", "Var Comps",0.,0., 800, 600);
    ...
    h_looseApprox->GetYaxis()->SetNdivisions(4,5,0); //4,5,0
    h_looseApprox->GetXaxis()->SetLabelOffset(0.028);
    h_looseApprox->GetYaxis()->SetTitleSize(0.08);
    h_looseApprox->GetYaxis()->SetTitleOffset(0.9);
    h_looseApprox->GetYaxis()->SetLabelSize(0.08);
    Double_t ymin;
    Double_t ymax;
    Double_t xmin;
    Double_t xmax;
...
// hdata_TTVA_looseNEW
// hdata_TTVA_looseOLD
            h_looseApprox->GetXaxis()->SetRangeUser(xmin, xmax);// along X
            h_looseApprox->SetMaximum(ymax);   // along          
            h_looseApprox->SetMinimum(ymin);  //   Y  
        // hdata_TTVA_looseOLD->Draw("e0p");
        // hdata_TTVA_looseNEW->Draw("e0p");
        // h_looseApprox->SetMarkerStyle(23);
        h_looseApprox->Draw("e0p");
...
leg2->Draw();
    ...
    if (drawResiduals) {
        pad2->cd();
        hres = (TH1F*)h_looseApprox->Clone("hres");
        hres->Sumw2();
        hres->Divide(h_looseApprox,h_looseTrkPerigee,1.0,1.0);
...
c2->SaveAs(OutdirName + "Figures/VarComps2020Mar6_VarBinned_v8_mixVars_looseD2/"+varDir+"/"+typeDir+"/VarComp_"+Tag+".pdf");
delete c2;
...
void OtherIso_MultiVar_TTVA_v8_looseD2() {
