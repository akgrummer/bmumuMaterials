...
ColorIndex = 4;
TH1F* h_custom_approx = RetrieveHist(DirName, theYear, DataType, B_iso_10_noChi2_LooSiHi1Pt05_T0004194304, ColorIndex, FileDate, myPileup);
ColorIndex = 2;
TH1F* h_customDerivation = RetrieveHist(DirName, theYear, DataType, B_iso_10_noChi2_LooSiHi1Pt05_T0134217728, ColorIndex, FileDate, myPileup);
ColorIndex = 1;
TH1F* h_TTVA_custom_drNEW = RetrieveHist(DirName, theYear, DataType, B_Iso_TTVA_custom_dr, ColorIndex, FileDate, myPileup);
ColorIndex = 5;
// DirName  =  "/eos/home-a/agrummer/BsmumuData/FinalSkim/v6/";
// TH1F* h_TTVA_custom_drOLD = RetrieveHist(DirName, theYear, DataType, 
//     B_Iso_TTVA_custom_dr, ColorIndex, "2019Dec5", myPileup);
...
// TH1F* h_1516 = RetrieveHist(theYear, DataType, B_iso_7_Chi2_5_LooSiHi1Pt05, ColorIndex, FileDate, myPileup);
// ColorIndex = 1;
// TH1F* h_asRun1 = RetrieveHist(theYear, DataType, B_iso_7_Chi2_5_LooSiHi1Pt05_T0000000127, ColorIndex, FileDate, myPileup);
// // ColorIndex = 5;
...
TCanvas* c2 = new TCanvas("c2", "Var Comps",0.,0., 800, 600);
    ...
    h_TTVA_custom_drNEW->GetYaxis()->SetNdivisions(4,5,0); //4,5,0
    h_TTVA_custom_drNEW->GetXaxis()->SetLabelOffset(0.028);
    h_TTVA_custom_drNEW->GetYaxis()->SetTitleSize(0.08);
    h_TTVA_custom_drNEW->GetYaxis()->SetTitleOffset(0.9);
    h_TTVA_custom_drNEW->GetYaxis()->SetLabelSize(0.08);
    // cout<<"DEBUG: 2"<<endl;
    Double_t ymin;
    Double_t ymax;
    Double_t xmin;
    ...
    if(DataType.EqualTo("data")){
        ...
        // h_TTVA_custom_drNEW
        // h_TTVA_custom_drOLD
        h_TTVA_custom_drNEW->SetMaximum(ymax);   // along          
        h_TTVA_custom_drNEW->SetMinimum(ymin);  //   Y  
        // h_TTVA_custom_drOLD->Draw("e0p");
        h_TTVA_custom_drNEW->Draw("e0p");
        h_custom_approx->SetMarkerStyle(23);
        h_custom_approx->Draw("e0psame");
        h_customDerivation->SetMarkerStyle(22);
        ...
        h_TTVA_custom_drNEW->SetMaximum(ymax);   // along          
        h_TTVA_custom_drNEW->SetMinimum(ymin);  //   Y  
        h_TTVA_custom_drNEW->SetMaximum(5.);
        // h_TTVA_custom_drOLD->Draw("hist e0");
        h_TTVA_custom_drNEW->Draw("hist e0");
        h_custom_approx->SetLineStyle(4);
        h_custom_approx->Draw("hist e0 same");
        h_customDerivation->SetLineStyle(2);
...
// leg2->AddEntry(h_customDerivation, "Custom TVA Trk Perigee, new nTuple, derivation","pe");
leg2->AddEntry(h_customDerivation, "BEJ","pe");
// leg2->AddEntry(h_custom_approx, "Custom TVA Approx., new nTuple, Derviation","pe");
leg2->AddEntry(h_custom_approx, "BFJ, derivation","pe");
// leg2->AddEntry(h_TTVA_custom_drNEW, "Custom TVA new nTuple, from Trks","pe");
leg2->AddEntry(h_TTVA_custom_drNEW, "BFJ, from trks","pe");
// leg2->AddEntry(h_TTVA_custom_drOLD, "Custom TVA old nTuple, from Trks","pe");
}else if(DataType.EqualTo("mc")){
// if (!myPileup.EqualTo("high"))
// leg2->AddEntry(h_customDerivation, "Custom TVA Trk Perigee, new nTuple, derivation","l");
leg2->AddEntry(h_customDerivation, "BEJ","l");
// leg2->AddEntry(h_custom_approx, "Custom TVA Approx., new nTuple, Derviation","l");
leg2->AddEntry(h_custom_approx, "BFJ, derivation","l");
// leg2->AddEntry(h_TTVA_custom_drNEW, "Custom TVA new nTuple, from Trks","l");
leg2->AddEntry(h_TTVA_custom_drNEW, "BFJ, from trks","l");
// leg2->AddEntry(h_TTVA_custom_drOLD, "Custom TVA old nTuple, from Trks","l");
    }
// cout<<"DEBUG: 6"<<endl;
...
leg2->Draw();
    ...
    TH1F *hres(NULL), *hres2(NULL);
    if (drawResiduals) {
        pad2->cd();
        hres = (TH1F*)h_TTVA_custom_drNEW->Clone("hres");
        hres->Sumw2();
        hres->Divide(h_custom_approx,h_TTVA_custom_drNEW,1.0,1.0);
        ...
        hres->SetMarkerColor(ColorWheel[4]);
        hres->SetLineColor(ColorWheel[4]);
        hres->SetMarkerStyle(23);
        hres->Draw("e0p same");

        hres2 = (TH1F*)h_TTVA_custom_drNEW->Clone("hres2");
        hres2->Sumw2();
        hres2->Divide(h_customDerivation,h_TTVA_custom_drNEW,1.0,1.0);
        // hres2->GetXaxis()->SetRangeUser(xmin, xmax);
        hres2->SetMaximum(1.01);   // along          
        hres2->SetMinimum(0.99);  //   Y 
        hres2->SetMarkerSize(0.7);
        hres2->GetXaxis()->SetTitleOffset(1.0); //0.95
        hres2->GetXaxis()->SetTitleSize(0.17);
        hres2->GetXaxis()->SetLabelSize(0.14); //0.17 0.1
        hres2->GetYaxis()->SetTitleSize(0.17);
        hres2->GetYaxis()->SetTitleOffset(0.47); //0.26
        hres2->GetYaxis()->SetLabelSize(0.1);//0.17 //0.08
        hres2->GetYaxis()->SetNdivisions(4,3,0); //4,5,0

        hres2->GetYaxis()->SetTitle("Ratio");
        // hres2->GetXaxis()->SetTitle(Xtitle);
        hres2->SetMarkerColor(ColorWheel[2]);
        hres2->SetLineColor(ColorWheel[2]);
        hres2->SetMarkerStyle(22);
        hres2->SetLineStyle(2);
        hres2->Draw("e0p same");
    }
...
c2->SaveAs(OutdirName + "Figures/VarComps2020Mar6_VarBinned_v8_mixVars_customD/"+varDir+"/"+typeDir+"/VarComp_"+Tag+".pdf");
delete c2;
delete h_custom_approx;
delete h_customDerivation;
delete h_TTVA_custom_drNEW;
// delete h_TTVA_custom_drOLD;
// delete hRun1;
// delete h_1516;
// delete h_asRun1;
// delete h_custom_approx;
...
void Iso_MultiVar_TTVA_v8_customD(){
    TString FileDate = "";
    // FileDate = "2019July30";
    // FileDate = "2019Nov21";
    // FileDate = "2019Dec5";
