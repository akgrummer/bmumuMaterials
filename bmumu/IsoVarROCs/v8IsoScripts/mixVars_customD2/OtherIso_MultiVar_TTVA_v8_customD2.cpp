...
TCanvas* c2 = new TCanvas("c2", "Var Comps",0.,0., 800, 600);
    ...
    h_customApprox->GetYaxis()->SetNdivisions(4,5,0); //4,5,0
    h_customApprox->GetXaxis()->SetLabelOffset(0.028);
    h_customApprox->GetYaxis()->SetTitleSize(0.08);
    h_customApprox->GetYaxis()->SetTitleOffset(0.9);
    h_customApprox->GetYaxis()->SetLabelSize(0.08);
    Double_t ymin;
    Double_t ymax;
    Double_t xmin;
    Double_t xmax;
...
// hdata_TTVA_customNEW
// hdata_TTVA_customOLD
            h_customApprox->GetXaxis()->SetRangeUser(xmin, xmax);// along X
            h_customApprox->SetMaximum(ymax);   // along          
            h_customApprox->SetMinimum(ymin);  //   Y  
        // hdata_TTVA_customOLD->Draw("e0p");
        // hdata_TTVA_customNEW->Draw("e0p");
        // h_customApprox->SetMarkerStyle(23);
        h_customApprox->Draw("e0p");
...
leg2->Draw();
    ...
    if (drawResiduals) {
        pad2->cd();
        hres = (TH1F*)h_customApprox->Clone("hres");
        hres->Sumw2();
        hres->Divide(h_customApprox,h_customTrkPerigee,1.0,1.0);
...
c2->SaveAs(OutdirName + "Figures/VarComps2020Mar6_VarBinned_v8_mixVars_customD2/"+varDir+"/"+typeDir+"/VarComp_"+Tag+".pdf");
delete c2;
...
void OtherIso_MultiVar_TTVA_v8_customD2() {
