...
ColorIndex = 1;
TH1F* h_TTVA_loose_dr = RetrieveHist(theYear, DataType, B_Iso_TTVA_loose_dr, ColorIndex, FileDate, myPileup);
ColorIndex = 4;
...
TCanvas* c2 = new TCanvas("c2", "Var Comps",0.,0., 800, 600);
    ...
    if(DataType.EqualTo("data")){
        ...
        h_TTVA_loose_dr->Draw("hist e0 same");
...
leg2->AddEntry(h2016data_Default, "As in 2015+2016","pe");
leg2->AddEntry(h_TTVA_loose_dr, "Loose TVA WP, #Delta R<1.0","pe");
leg2->AddEntry(h_TTVA_custom_dr, "Custom TVA WP, #Delta R<1.0","pe");
leg2->AddEntry(h_all_c5, "All Tracks and #chi^{2}_{PV} #leq 5","pe");
}else if(DataType.EqualTo("mc")){
if (!myPileup.EqualTo("high")){leg2->AddEntry(hRun1, "Run 1","l");}
leg2->AddEntry(h2016data_Default, "As in 2015+2016","l");
leg2->AddEntry(h_TTVA_loose_dr, "Loose TVA WP, #Delta R<1.0","l");
leg2->AddEntry(h_TTVA_custom_dr, "Custom TVA WP, #Delta R<1.0","l");
...
c2->SaveAs(OutdirName + "Figures/VarComps2020Feb19_VarBinned_v8/"+varDir+"/"+typeDir+"/VarComp_"+Tag+".pdf");
delete c2;
delete hRun1;
delete h2016data_Default;
delete h_TTVA_custom_dr;
delete h_TTVA_loose_dr;
delete h_all_c5;
return 0;
}

void Iso_MultiVar_TTVA_v8(){
    TString FileDate = "";
    // FileDate = "2019July30";
    // FileDate = "2019Nov21";
    // FileDate = "2019Dec5";
