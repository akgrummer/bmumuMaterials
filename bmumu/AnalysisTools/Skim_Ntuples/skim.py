...
#import background_classification //FIXME
#import validation
#-------------------------------------------
...
#-------------------------------------------
def print_arguments():
    print("Input file: %s" % l_FILE)
    print("Output file: %s" % OUTPUT)
    print("Process: %s" % PROCESS)
    print("Nevents: %s" % nevs)
#-------------------------------------------
#Initialize
#-------------------------------------------
ROOT.gROOT.SetBatch()
...
args_steer = sys.argv
if len(args_steer) == 6:
    l_FILE.append(sys.argv[-4])
    OUTPUT=sys.argv[-3]
    PROCESS=sys.argv[-2]
    nevs=int(sys.argv[-1])
elif len(args_steer) == 5:
    l_FILE.append(sys.argv[-3])
    OUTPUT=sys.argv[-2]
    PROCESS=sys.argv[-1]
    nevs=-1
else:
