...
// author2: Aidan(dot)Grummer(at)cern(dot)ch
//
// to run:
...
//
//
//Feb 24, 2020: use for redoing v6 of 2019 ntuples, change output directory and:
//med variables are for comparisons to Run1 - they were already back in this script.
//renamed these four variables: muP_iso_7_Chi2_5_LoosePt05, muM_iso_7_Chi2_5_LoosePt05  back to
// muP_iso_7_chi2_5_LoosePt05 muM_iso_7_chi2_5_LoosePt05
//same for (upper to lower C) muP_iso_Ntracks_7_chi2_5_LoosePt05, muM_iso_Ntracks_7_chi2_5_LoosePt05;
//
//March 3, back to capital C on these files
//med variables are use to compare to run 1 files -- these have a lower case c
// use _oldnTuples.cxx version for the other form from now on.
//
//March 16, used for v3 nTuples
...
void loopFiles(TString indir, TString filter=""){
  ...
  //       outdir = "/eos/home-a/agrummer/BsmumuData/2020nTuples/FinalSkim/v2";}//2020/v2
  // else {outdir = "/eos/home-a/agrummer/BsmumuData/2020nTuples/FinalSkim/v1";}//2020/v1
  outdir = "/eos/home-a/agrummer/BsmumuData/2020nTuples/FinalSkim/v3";
  ...
  if (dpdf != NULL){
    while (entry = readdir(dpdf)){
      ...
      if ( hasEnding(entry->d_name, "merged.root") )  continue;  // merged.root file
...
// cout<< "Entries in original tree: "<< ts->GetEntries() <<endl;
  ...
  double B_minChi2MuonsIPtoAnyPV, closeTrkDOCA;
  ...
  for (auto ievent=0; ievent<nevents; ievent++)
    {
      ...
      if (!Muon1_accept || !Muon2_accept || 
        !Muon1_isCombinedMuon || !Muon2_isCombinedMuon){
        MuonConditionsCut+=1; 
        continue;}
    ...
    pTThreshold2 = 4.0;
      ...
      */
      ...
      closeTrkDOCA = (double) sqrt(pow(closeTrkDCA,2)+pow(closeTrkZCA,2));
    ...
    cout<< "MuonConditionsCut: " << MuonConditionsCut<< endl;
