...
TCanvas* IsolationStudy_fileloop(TString myYear="", TString nTupleVer = ""){
  ...
  if (nTupleVer.EqualTo("new")){
      indir = "/eos/home-a/agrummer/BsmumuData/2020nTuples/FinalSkim/v1/";}
      else{
          indir = "/eos/home-a/agrummer/BsmumuData/FinalSkim/v6/";}
  // int myYear(2018);
  ...
  dpdf = opendir(finpath.Data());
    ...
    h_numTrks   = new TH1F("h_numTrks","chi2 to associated PV for selected tracks",1000,0,1000);
...
for (auto ievent=0; ievent<nevents; ievent++)
    {
        ...
        //             }
        //         // // loose
        //         //     if (h_PIXhits_Loose) h_PIXhits_Loose->Fill((*idtrk_nPIXhits).at(idx));
        //         //     if (h_SCThits_Loose) h_SCThits_Loose->Fill((*idtrk_nSCThits).at(idx));
        //         //     if (h_IBLhits_Loose) h_IBLhits_Loose->Fill((*idtrk_NIBLayer).at(idx));

        //         //  //isTightPrimary
        //         //   if ( (*idtrk_isTightPrimary).at(idx)==1){
        //         //       if (h_PIXhits_isTight) h_PIXhits_isTight->Fill((*idtrk_nPIXhits).at(idx));
        //         //       if (h_SCThits_isTight) h_SCThits_isTight->Fill((*idtrk_nSCThits).at(idx));
        //         //       if (h_IBLhits_isTight) h_IBLhits_isTight->Fill((*idtrk_NIBLayer).at(idx));
        //         //   }
        //         //   //like run 1
        //         //    if (((*idtrk_nPIXhits).at(idx) - (*idtrk_NIBLayer).at(idx))>=1
        //         //       && (*idtrk_nSCThits).at(idx)>=2 ){
        //         //     if (h_PIXhits_Run1) h_PIXhits_Run1->Fill((*idtrk_nPIXhits).at(idx));
        //         //     if (h_SCThits_Run1) h_SCThits_Run1->Fill((*idtrk_nSCThits).at(idx));
        //         //     if (h_IBLhits_Run1) h_IBLhits_Run1->Fill((*idtrk_NIBLayer).at(idx));
        //         //   }
        //         //   //like run 2
        //         //    if (((*idtrk_nPIXhits).at(idx))>=1
        //         //       && (*idtrk_nSCThits).at(idx)>=2 ){
        //         //     if (h_numTrks) h_numTrks->Fill((*idtrk_nPIXhits).at(idx));
        //         //     if (h_SCThits_Run2) h_SCThits_Run2->Fill((*idtrk_nSCThits).at(idx));
        //         //     if (h_IBLhits_Run2) h_IBLhits_Run2->Fill((*idtrk_NIBLayer).at(idx));
        //         //   }
...
void RunOverData(TString inYear ="", TString nTupleVer = ""){
    ...
    myC->SaveAs(myDir+"data/"+inYear+"_numTrks_2020Mar3"+nTupleVer+".root");
    myC->SaveAs(myDir+"figures/"+inYear+"_numTrks_2020Mar3"+nTupleVer+".png");

}

void RunOverSavedFiles_Chi2(TString theYear=""){
    SetAtlasStyle();
    ...
    TFile* ifile_numTrks_new = new TFile(myDir+theYear+"_numTrks_2020Mar3new.root");
    TCanvas *Canvas_numTrks_new = (TCanvas*)ifile_numTrks_new->Get("trks chi2");

    TH1F *h_numTrks_new = (TH1F*)Canvas_numTrks_new->GetListOfPrimitives()->FindObject("h_numTrks");
    // TH1F *h_SCThits_Run2_new = (TH1F*)Canvas_numTrks_new->GetListOfPrimitives()->FindObject("h_SCThits_Run2");

    ifile_numTrks_new->Close();

    TFile* ifile_numTrks_old = new TFile(myDir+theYear+"_numTrks_2020Mar3old.root");
    TCanvas *Canvas_numTrks_old = (TCanvas*)ifile_numTrks_old->Get("trks chi2");

    TH1F *h_numTrks_old = (TH1F*)Canvas_numTrks_old->GetListOfPrimitives()->FindObject("h_numTrks");
    // TH1F *h_SCThits_Run2_old = (TH1F*)Canvas_numTrks_old->GetListOfPrimitives()->FindObject("h_SCThits_Run2");

    ifile_numTrks_old->Close();
...
h_numTrks_new->SetLineColor(kRed);
h_numTrks_new->SetLineStyle(2);
h_numTrks_new->SetMarkerSize(0.);
// h_numTrks_new->SetMarkerStyle(22.);
// h_numTrks_new->SetMarkerColor(kRed);
...
h_numTrks_old->SetLineColor(kGreen+2);
h_numTrks_old->SetMarkerSize(0.);
h_numTrks_old->SetLineStyle(3.);
// h_numTrks_old->SetMarkerStyle(23.);
// h_numTrks_old->SetMarkerColor(kGreen+2);
// h_SCThits_Run2_old->SetLineColor(kGreen+2);
// h_SCThits_Run2_old->SetMarkerSize(0.);
// h_SCThits_Run2_old->SetLineStyle(3.);
// h_SCThits_Run2_old->SetMarkerStyle(23.);
// h_SCThits_Run2_old->SetMarkerColor(kGreen+2);
...
// h_IBLhits_Run2->SetMarkerSize(0.);
    ...
    // // if (h_my_iso_nonAss_2017_numTrks) h_my_iso_nonAss_2017_numTrks->Draw("e0 hist same");
    // if (h_my_iso_all_2017_numTrks) h_my_iso_all_2017_numTrks->Draw("e0 hist same");
    ...
    // // if (h_my_iso_nonAss_2018_numTrks) h_my_iso_nonAss_2018_numTrks->Draw("e0 hist same");
    // if (h_my_iso_all_2018_numTrks) h_my_iso_all_2018_numTrks->Draw(" e0 hist same");
    // //for logy scale
    // gPad->SetLogy();
...
// if(Detector.EqualTo("PIX")) {
    h_numTrks_old->GetXaxis()->SetRangeUser(0, 1001);
    // ymin = 1e-1;
    // ymax = 1e7;
    // h_numTrks_old->SetMaximum(ymax);   // along          
    // h_numTrks_old->SetMinimum(ymin);  //   Y  
    ...
    h_numTrks_old->Draw("e0 hist");
    h_numTrks_new->Draw("e0 hist same");
    h_numTrks_old->GetXaxis()->SetTitle("Num. of PIX Hits");
    h_numTrks_old->GetYaxis()->SetTitle("Candidates #times Num. of Trks per cand.");
...
//tight label doesn't matter here as long as the line colors don't change
        ...
        legAll->AddEntry(h_numTrks_new, "New nTuple" ,"l");
        legAll->AddEntry(h_numTrks_old, "Old nTuple" ,"l");
    ...
    // allC->SaveAs("/eos/home-a/agrummer/www/bmumu/AnalysisTools/Examples/alltrksExample/figures/Iso/IsoComp_wNonAssoc_numTrks.png");
    // allC->SaveAs("/eos/home-a/agrummer/www/bmumu/AnalysisTools/Examples/alltrksExample/figures/Iso/IsoComp_wNonAssoc_numTrks_linear.png");
    // allC->SaveAs("/eos/home-a/agrummer/www/bmumu/AnalysisTools/Examples/alltrksExample/figures/Iso/IsoComp_allTrks_Chi2_5.png");
    // allC->SaveAs("/eos/home-a/agrummer/www/bmumu/AnalysisTools/Examples/alltrksExample/figures/Iso/IsoComp_allTrks_numTrks_Chi2_5.png");
    // allC->SaveAs("/eos/home-a/agrummer/www/bmumu/AnalysisTools/Examples/alltrksExample/figures/Iso/IsoComp_allTrks_numTrks_Chi2_4.png");
    ...
    // allC->SaveAs("/eos/home-a/agrummer/www/bmumu/AnalysisTools/Examples/alltrksExample/figures/numTrks/"+theYear+"_"+Detector+"_2019Jun13.png");
    allC->SaveAs("/eos/home-a/agrummer/www/bmumu/AnalysisTools/Examples/alltrksExample/figures/numTrks/"+theYear+"_2020Mar3.png");
}
...
//Second configuration:
   ...
   // // nonAss
   // // assPV
  // RunOverSavedFiles_Chi2(pattern);
  return 0;
}
