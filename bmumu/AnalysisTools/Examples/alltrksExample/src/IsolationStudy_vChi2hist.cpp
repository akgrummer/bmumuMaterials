...
for (auto ievent=0; ievent<nevents; ievent++)
    {
        ...
        for (unsigned int idx=0; idx<ntrks; idx++) 
        {
            ...
            if (
                ...
                if (h_chi2_VTX_all) h_chi2_VTX_all->Fill( (*B_VTX_trksChi2).at(idx) );
                if (h_chi2_PV_all) h_chi2_PV_all->Fill( (*B_PV_trksChi2).at(idx) );
...
TCanvas* c = new TCanvas("trks chi2","trks chi2",900,600);
  if (doIsoStudy)  {
    ...
    if (h_chi2_VTX_otherPV)h_chi2_VTX_otherPV->Draw("same");
...
//
    ...
    myC->SaveAs(myDir+"data/"+"2017chi2_2019May7.root");
    myC->SaveAs(myDir+"figures/"+"2017chi2_2019May7.png");

}

void RunOverSavedFiles_Chi2(){
    SetAtlasStyle();
    ...
    TFile* ifile2017_chi2 = new TFile(myDir+"2017chi2_2019May7.root");
    TCanvas *Canvas2017_chi2 = (TCanvas*)ifile2017_chi2->Get("trks chi2");

    TH1F *h_chi2_VTX_all = (TH1F*)Canvas2017_chi2->GetListOfPrimitives()->FindObject("h_chi2_VTX_all");
    // h_chi2_VTX_all->Scale(1./(h_chi2_VTX_all->Integral(0, numberOfBinsChi2 + 1)));
    TH1F *h_chi2_VTX_otherPV = (TH1F*)Canvas2017_chi2->GetListOfPrimitives()->FindObject("h_chi2_VTX_otherPV");
    // h_chi2_VTX_otherPV->Scale(1./(h_chi2_VTX_otherPV->Integral(0, numberOfBinsChi2 + 1)));
    TH1F *h_chi2_VTX_nonAss = (TH1F*)Canvas2017_chi2->GetListOfPrimitives()->FindObject("h_chi2_VTX_nonAss");
    // h_chi2_VTX_nonAss->Scale(1./(h_chi2_VTX_nonAss->Integral(0, numberOfBinsChi2 + 1)));
    TH1F *h_chi2_VTX_assPV = (TH1F*)Canvas2017_chi2->GetListOfPrimitives()->FindObject("h_chi2_VTX_assPV");
    // h_chi2_VTX_assPV->Scale(1./(h_chi2_VTX_assPV->Integral(0, numberOfBinsChi2 + 1)));

    TH1F *h_chi2_PV_all = (TH1F*)Canvas2017_chi2->GetListOfPrimitives()->FindObject("h_chi2_PV_all");
    // h_chi2_PV_all->Scale(1./(h_chi2_PV_all->Integral(0, numberOfBinsChi2 + 1)));
    TH1F *h_chi2_PV_otherPV = (TH1F*)Canvas2017_chi2->GetListOfPrimitives()->FindObject("h_chi2_PV_otherPV");
    // h_chi2_PV_otherPV->Scale(1./(h_chi2_PV_otherPV->Integral(0, numberOfBinsChi2 + 1)));
    TH1F *h_chi2_PV_nonAss = (TH1F*)Canvas2017_chi2->GetListOfPrimitives()->FindObject("h_chi2_PV_nonAss");
    // h_chi2_PV_nonAss->Scale(1./(h_chi2_PV_nonAss->Integral(0, numberOfBinsChi2 + 1)));
    TH1F *h_chi2_PV_assPV = (TH1F*)Canvas2017_chi2->GetListOfPrimitives()->FindObject("h_chi2_PV_assPV");
    // h_chi2_PV_assPV->Scale(1./(h_chi2_PV_assPV->Integral(0, numberOfBinsChi2 + 1)));
    ////
    ...
    h_chi2_PV_all->SetLineColor(kRed);
    h_chi2_PV_all->SetMarkerSize(0.);
    ...
    if (h_chi2_PV_all) h_chi2_PV_assPV->Draw("e0 hist same");
    if (h_chi2_PV_all) h_chi2_PV_otherPV->Draw("e0 hist same");
    if (h_chi2_PV_all) h_chi2_PV_nonAss->Draw("e0 hist same");
    h_chi2_PV_all->GetXaxis()->SetTitle("#chi^{2}");
    // h_chi2_PV_all->GetYaxis()->SetTitle("a.u.");
    h_chi2_PV_all->GetYaxis()->SetTitle("Candidates * Num. of Trks per cand.");
    ...
    legAll->AddEntry(h_chi2_PV_all, "All Tracks" ,"l");
    legAll->AddEntry(h_chi2_PV_assPV, "Tracks Associated to B PV" ,"l");
    legAll->AddEntry(h_chi2_PV_otherPV, "Tracks Assoc. to B PV and other PVs" ,"l");
    legAll->AddEntry(h_chi2_PV_nonAss, "Tracks Assoc. to B PV and not Assoc. to any PV" ,"l");
