...
void compare(VariableInfo, TString, vector <TChain*>,vector <TString>, vector <TChain*>,vector <TString>, vector <TChain*>,vector <TString>, TString, TString, bool, bool drawResiduals=true);
...
void dmcbbmumuX_v2(TString compVar="", TString compMode="", Bool_t verbose=true, TString period = "")
{
...
//  cout << "You entered: " << input << endl << endl;
 ...
 if (mode.EqualTo("mc_mc")) {
    ...
    h1title+="_";
    h1title+=treenames[i];
    histogram[i] = getHistoFromTree(chains[i], Variable, h1title, (CutAdd && CutSB));
    h1title = h1titledummy;
    h1title+="_dt16";
    } 
    h2title+="_mcX";
...
if (NumberOfTreesSigMC>1){
  for (int i=0;i<NumberOfTreesSigMC;i++){
  elementsSigMC.push_back(h2[i]->GetMaximum());
}
...
//------------------------------------------------------------------------
  ...
  if (Variable.nbins>0 && Variable.XMin!= Variable.XMax) {
    ...
    if (h3[maxintegerSigMC]) h2[maxintegerSigMC]->GetXaxis()->SetRangeUser(Variable.XMin,Variable.XMax);
  }
  ...
  if (normalize) {
    ...
    if (h2[maxintegerSigMC]) cout << " Signal MC:  "<< h2[maxintegerSigMC]->GetTitle()<< ": "<< h2[maxintegerSigMC]->Integral(0, Variable.nbins + 1);
    cout << endl;
    cout << endl;
  ...
  if (drawResiduals) {
    ...
    if (h3[maxintegerSigMC]) {
      h3[maxintegerSigMC]->GetYaxis()->SetTitleOffset(0.72);
      h3[maxintegerSigMC]->GetYaxis()->SetTitleSize(0.07);
    }
  }
...
if (arbitraryUnits){
    if (h1title.Contains("B_minChi2MuonsIPtoAnyPV")){
        histogram[maxinteger]->GetYaxis()->SetTitle("Events / 0.25");
        if (h2[maxintegerMC]) h2[maxintegerMC]->GetYaxis()->SetTitle("Events / 0.25");
        if (h3[maxintegerSigMC]) h3[maxintegerSigMC]->GetYaxis()->SetTitle("Events / 0.25");
    }
    else if (h1title.Contains("fabs_a_2D")){
        histogram[maxinteger]->GetYaxis()->SetTitle("Events / 0.02 rad");
        if (h2[maxintegerMC]) h2[maxintegerMC]->GetYaxis()->SetTitle("Events / 0.02 rad");
        if (h3[maxintegerSigMC]) h3[maxintegerSigMC]->GetYaxis()->SetTitle("Events / 0.02 rad");
    }
    else{
...
if (h3[maxintegerSigMC]){
  h3[maxintegerSigMC]->SetMarkerColor(ColorWheel[1]);
  h3[maxintegerSigMC]->SetLineColor(ColorWheel[1]);
  h3[maxintegerSigMC]->SetLineWidth(2);
  h3[maxintegerSigMC]->SetFillColor(ColorWheel[1]);
  h3[maxintegerSigMC]->SetFillStyle(3007);
  // h3[maxintegerSigMC]->SetFillStyle(3003);
}
// h2[maxintegerMC]->SetLineColor(ColorWheel[0]);

h2[maxintegerMC]->Draw("e0hist");
// h2[maxintegerMC]->Draw("ehist");
for (int i=0;i<NumberOfTreesMC;i++) {
  if (i != maxintegerMC){
   // h2[i]->SetLineColor(ColorWheel[i+1]);
   h2[i]->Draw("e0histsame");
  }
}
if(h3[maxintegerSigMC]){
for (int i=0;i<NumberOfTreesSigMC;i++) {
...
if (!Variable.logScale){
  ...
  // }else{
    ...
    if (h1title.Contains("B_minChi2MuonsIPtoAnyPV")) {
        histogram[maxinteger]->GetYaxis()->SetRangeUser(YminVal/1.5, YmaxVal/2*1.05);
        h2[maxintegerMC]->GetYaxis()->SetRangeUser(YminVal/1.5, YmaxVal/2*1.05);
        h3[maxintegerSigMC]->GetYaxis()->SetRangeUser(YminVal/1.5, YmaxVal/2*1.05);
    }
  // }
}else {
  histogram[maxinteger]->SetMaximum(YmaxVal*10);
  h2[maxintegerMC]->SetMaximum(YmaxVal*10);
  h3[maxintegerSigMC]->SetMaximum(YmaxVal*10);
...
if (h1title.Contains("B_minChi2MuonsIPtoAnyPV")) atlasLabel(0.2, 0.54, true, true, 1, true, false, true, h1title);
else if (h1title.Contains("fabs_a_2D")) {
    atlasLabel(0.21, 0.75, true, true, 1, true, false, true, h1title);
}
else atlasLabel(0.64, 0.81, true, true, 1, false, false, true, h1title);
    ...
    if (mode.Contains("dtbgsub_mcrew")) {
      ...
      for (int i=0;i<NumberOfTrees;i++) {
        ...
        leg->AddEntry(histogram[i],mclabeldummy,"elp");
         ...
         if(!Variable.PlotOnlyMC) leg->AddEntry(histogram[i],"B_{s} #rightarrow #mu^{+} #mu^{-} sidebands","p");
        if (NumberOfTreesMC>1){ 
      ...
      leg->AddEntry(h3[0],"B_{s} #rightarrow #mu^{+} #mu^{-} MC","fl");
    }
    }
  }
...
TH1F** hres = new TH1F*[NumberOfTrees];
  ...
  if (drawResiduals) {
    ...
    hres[i]->GetYaxis()->SetTitleOffset(0.47); //0.26
    ...
    hres[i]->GetYaxis()->SetTitle("Data / bkg");
...
void atlasLabel(Double_t x,Double_t y, bool drawRes = true, bool Preliminary=true,Color_t color=1, bool drawEnergy=false, bool drawLumi=false, bool enlarge=true, TString h1title = "")
{
  ...
  if (drawEnergy) {
    if (h1title.Contains("fabs_a_2D")) tex = new TLatex(0.59,0.55,"#sqrt{s} = 13 TeV, 26.3 fb^{-1}");
...
int NumberOfColumns(2);
  ...
  if (mode.EqualTo("dt_mcXs") || mode.EqualTo("dt_mcXs_Mult") || mode.EqualTo("dt_mc_OnePeriod")){
    ...
    if (what.Contains("chi2_PVSV_log2D")) {
        ...
        leg = new TLegend(0.530075, 0.67237, 0.888471, 0.862187, NULL,"brNDC");}
    else leg = new TLegend(0.577694, 0.604764, 0.93609, 0.797181, NULL,"brNDC"); //xbl(0.71),ybl(0.62),xur(0.98),yur(0.98)
  } else {
    if (what.Contains("Iso") || what.Contains("d0") || what.Contains("Eta") || what.Contains("eta") || what.Contains("DCA") || what.Contains("BDT")) leg = new TLegend(0.15,0.64,0.68,0.92,NULL,"brNDC"); //xbl,ybl,xur,yur //0.09
    else if (what.Contains("DR")) leg = new TLegend(0.45,0.74,0.68,0.92,NULL,"brNDC");
    // else leg = new TLegend(0.74,0.75,0.95,0.93,NULL,"brNDC"); //xbl(0.71,0.68),ybl(0.62),xur(0.98),yur(0.98)
    else leg = new TLegend(0.602757, 0.581361, 0.927318, 0.739976,NULL,"brNDC"); //xbl(0.71,0.68),ybl(0.62),xur(0.98),yur(0.98)
  }
