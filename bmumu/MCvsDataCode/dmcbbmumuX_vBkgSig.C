...
void dmcbbmumuX_vBkgSig(TString compVar="", TString compMode="", Bool_t verbose=true, TString period = "")
{
...
// TChain* td = chains[0];
  ...
  for (int i=0;i<nVar;i++) {
    ...
    for (unsigned int j=0;j<Modes.size();j++) {
      ...
      compare(Variables[i], Modes[j], chains, treenames, chainsMC, treenamesMC, chainsSigMC, treenamesSigMC, dtlabel, mclabel, true, false);
    }
  }
  fout->cd();
  fout->Close();
