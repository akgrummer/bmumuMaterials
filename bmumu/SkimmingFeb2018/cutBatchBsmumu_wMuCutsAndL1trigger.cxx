...
double computeWeight(double pt, double eta, TH2D*QLC, TH1D*DDWpt, TH1D*DDWeta){
  ...
  if (wQLC!=0.) totalweight = (wDDWpt*wDDWeta)/wQLC;
  else cout << "warning, wQLC=0, setting weight to "<< totalweight << endl;
...
void cutBatchBsmumu(TString indir=""){
  ...
  */
  ...
  /*
  cout << "loading histograms with weights" << endl;
  TFile* QLCinput = new TFile(QLCfileName, "READ");
  QLCinput->cd();
  cout << "loading QLC" << endl;
  TH2D* QLC = (TH2D*)QLCinput->Get("h_QLC_BsMuMu");
  if (QLC) QLC->SetDirectory(0);
  else {
    cout << "could not find QLC histo in file "<< QLCfileName << " exiting" << endl;
    return;
  }
  if (QLCinput) QLCinput->Close();

  cout << "loading histograms with DDW" << endl;
  TFile * DDWinput = new TFile(DDWfileName,"READ");
  DDWinput->cd();
  TH1D* combinedPtDDW  = (TH1D*) DDWinput->Get("h_DDWpt_even_BpJpsiK");
  if (combinedPtDDW) combinedPtDDW->SetDirectory(0);
  else {
    cout << "could not find combinedPtDDW histo in file "<< DDWfileName << " exiting" << endl;
    return;
  }
  TH1D* combinedEtaDDW = (TH1D*) DDWinput->Get("h_DDWeta_even_BpJpsiK");
  if (combinedEtaDDW) combinedEtaDDW->SetDirectory(0);
  else {
    cout << "could not find combinedEtaDDW histo in file "<< DDWfileName << " exiting" << endl;
    return;
  }
  DDWinput->Close();
  ////////////////////////////////////////////////////////
  ...
  */
  ...
  fspath.Append("/skim/skim*.root");  // OLD MC
  ...
  for (auto ievent=0; ievent<nevents; ievent++)
    {
      ...
      // bool pass_trigger = ( HLT_trigger->find("HLT_mu6_mu4_bBmumu_Lxy0")==HLT_trigger->end() && HLT_trigger->find("HLT_2mu6_bBmumu_Lxy0")!=HLT_trigger->end());
      // bool pass_trigger = ( HLT_trigger->find("HLT_2mu4_bBmumu")!=HLT_trigger->end() && HLT_trigger->find("HLT_mu6_mu4_bBmumu")==HLT_trigger->end() && HLT_trigger->find("HLT_2mu6_bBmumu")==HLT_trigger->end());
      ...
      //if ((*decay) != "combinatorial") continue;
      ...
      //FIXME MC SIg only
      //
      //      CombWeights = computeWeight(B_pT, B_eta, QLC , combinedPtDDW, combinedEtaDDW);
