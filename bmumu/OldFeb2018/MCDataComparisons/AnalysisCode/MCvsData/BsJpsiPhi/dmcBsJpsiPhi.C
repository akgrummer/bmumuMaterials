// Macro for performing data/MC comparison on Bs->Jpsi/Phi channel
...
// 07.08.2017
//
// in the file dmcBsJpsiPhi.h are defined:
...
//
// 2. adjust CutAdd variable (in dmcBsJpsiPhi.h) if needed (additional cuts to be applied)
//
// 3. adjust output path for the plots, etc.
//
// 4. to exectute do (after "root -l"):
// .x dmcBsJpsiPhi.C("B_pT","dtbgsub_mcrew") //or any other variable name from dmcBsJpsiPhi.h file
// OR
// .x dmcBsJpsiPhi.C("systVars","dtbgsub_mcrew") // to run on all variables used for discrimination
// OR (faster, but on some OS stops after the first step - t.b.i)
// .L dmcBsJpsiPhi.C++
// dmcBsJpsiPhi("systVars","dtbgsub_mcrew")
// (N.B. in "dt_mc" mode histograms are compared "as is" i.e. w/o bkg subtraction)
//


#include "dmcBsJpsiPhi.h"
using namespace std;
...
Double_t exp_fn(Double_t* x,Double_t* par){
  return par[0]*TMath::Exp(par[1]*(x[0]-4966.0)); //FIXME 4966 - configurable
};
...
void dmcBsJpsiPhi(TString compVar="", TString compMode="", TString dtlabel="MC B^{+}#rightarrow J/#psi K^{+}", TString mclabel="MC B_{s}#rightarrow #mu #mu", Bool_t verbose=true)
{
  ...
  fd->cd();
  if (compMode.Contains("dtbgsub")) getNbkgBsJpsiPhi(td);
  //========================================================================================== 
...
void getNbkgBsJpsiPhi(TTree* td)
{
  cout << endl;
  cout << "Performing BsJpsiPhi fit to estimate # of signal events in signal region. Enough time to grap a coffee..." << endl;
  cout << endl;
  //
  ...
  histo->SetMinimum(0);
  histo->GetXaxis()->SetTitle("m_{J/#psi #phi} [MeV]");
  histo->GetYaxis()->SetTitleOffset(0.75);
  histo->GetYaxis()->SetTitle("B_{s} candidates / (10 MeV)");
  cout << endl;
  ...
  // with a double Gaussian function + exponential
  //----------------------------------------------------


  Double_t defInitFitPars[Npars];
  if (useEXP) {
    defInitFitPars[0] = 5000;
    defInitFitPars[1] = -0.0007;

    defInitFitPars[2] = 800.0;
    defInitFitPars[3] = 5366.0;
    defInitFitPars[4] = 15.0;

  } else {
    defInitFitPars[0] = 36000.0; //30K
    defInitFitPars[1] = -2.8;

    defInitFitPars[2] = 5500.0;
    defInitFitPars[3] = 6500.0;
    defInitFitPars[4] = 5280.0;
  }
  ...
  if (useEXP) {
    ...
    functot->SetParLimits(1,-0.5,0.00001);
    //2 dgaus norm 1st
    //3 dgaus norm 2nd
    //4 dgaus mu
    //5 dgaus sig1
    //6 dgaus sig2
    functot->SetParLimits(2, 1.0,10000.0);
    functot->SetParLimits(3, 5316.0,5416.0);   
    functot->SetParLimits(4, 1.0,20.0);
    //    functot->SetParLimits(6, 20.0,100.0);
  } else {
    //0 cont bkg exp norm
    //1 cont bkg exp slope
    functot->SetParLimits(0, 1.0,100000.0);
    ...
    //6 dgaus sig2
    functot->SetParLimits(2, 1.0,10000.0);
    functot->SetParLimits(3, 1.0,10000.0);   
    functot->SetParLimits(4, 5260.0,5300.0);
    //functot->SetParLimits(5, 10.0,50.0);
    //functot->SetParLimits(6, 35.0,100.0);
  } 
  ...
  TF1 *gaus1 = new TF1("gaus1",single_gaus,BLowerMassCut,BUpperMassCut,Npars); 
  gaus1->SetParameter(0,functot->GetParameter(2));
  gaus1->SetParameter(1,functot->GetParameter(4));
  gaus1->SetParameter(2,functot->GetParameter(5));
  gaus1->SetLineColor(kGreen);
  gaus1->Draw("same");
 
  /* // second gauss
  TF1 *gaus2 = new TF1("gaus2",single_gaus,BLowerMassCut,BUpperMassCut,Npars);
  gaus2->SetParameter(0,functot->GetParameter(3));
  gaus2->SetParameter(1,functot->GetParameter(4));
  gaus2->SetParameter(2,functot->GetParameter(6));
  gaus2->SetLineColor(kRed);
  gaus2->Draw("same"); 
  */
  const int totNBkgPars = 2;
  TF1 *totbkg = new TF1("totbkg",exp_fn,BLowerMassCut,BUpperMassCut,totNBkgPars);
  for(int i=0;i<totNBkgPars;i++) totbkg->SetParameter(i,functot->GetParameter(i));
  totbkg->SetLineColor(kOrange);
  totbkg->Draw("same");

  const int totNSigPars = 3; //FIXME: now single_gaus
  TF1 *totsig;
  totsig = new TF1("totsig",single_gaus,BLowerMassCut,BUpperMassCut,totNSigPars); //FIXME: now single_gaus
  ...
  Double_t NContBkgSR = totbkg->Integral(SRLow,SRHigh)/binWidth;
  Double_t NContBkgLSB = totbkg->Integral(LSBLow,LSBHigh)/binWidth;
  Double_t NContBkgRSB = totbkg->Integral(RSBLow,RSBHigh)/binWidth;
  ...
  for (int i=0;i<totNBkgPars;i++) totbkgpars[i]=functot->GetParameter(i);
  ...
  Double_t NContBkgSRErr = totbkg->IntegralError(SRLow,SRHigh,totbkgpars,totbkgmat)/binWidth;
  Double_t NContBkgLSBErr = totbkg->IntegralError(LSBLow,LSBHigh,totbkgpars,totbkgmat)/binWidth;
  Double_t NContBkgRSBErr = totbkg->IntegralError(RSBLow,RSBHigh,totbkgpars,totbkgmat)/binWidth;
  ...
  cout << "Total N bkg in SR: " << NContBkgSR << " pm: " << NContBkgSRErr << endl;
  ...
  nbkgInfo.nTotBkgSR = NContBkgSR;
  nbkgInfo.nTotBkgSRErr = NContBkgSRErr;

  nbkgInfo.nPartBkgSR = 0;
  nbkgInfo.nPartBkgSRErr = 0;
  ...
  //
  TPaveLabel *labBkg = new TPaveLabel(0.2,0.8,0.4,0.88, Form("N_{bkg}^{SR} = %4.1f #pm %3.1f", NContBkgSR, NContBkgSRErr),"rNDC");
...
Double_t FuncTotWithLin(Double_t *x, Double_t *par){  // total fit
  return lin_fn(x,par)+single_gaus(x,&par[2]); //FIXME: now single_gaus
}

Double_t FuncTotWithExp(Double_t *x, Double_t *par){  // total fit
  return exp_fn(x,par)+single_gaus(x,&par[2]);  //FIXME: now single
}
...
TH1F* getHistoFromTree(TTree* tree, const char* var, const char* hname, TCut cut, Int_t NBins, Float_t XMin, Float_t XMax)
{
  if (NBins==-1) {
    ...
    return NULL;
  } else cout << "simple GetVarHistoFromTree, getting histo "<< hname << " for variable " << var << " from tree "<< tree->GetName()<<" , NBins = " << NBins << endl;
  TString tmp = var;
  tmp.Append(">>");
  tmp.Append(hname);
  if (NBins>0 && XMin!=XMax) {
...
void compare(VariableInfo Variable, TString mode, TTree* td, TTree* tm, TString dtlabel, TString mclabel, bool normalize, bool drawResiduals)
{
  ...
  if (h2) {
    if (mode.Contains("dtbgsub_mcrew")) {
      ...
      leg->AddEntry(h2,"B_{s}#rightarrow J/#psi #phi MC15","l");
    } else {
      dtlabel+=" (A)";
      mclabel+=" (B)";
