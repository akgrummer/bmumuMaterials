...
// muP_Iso_all_c6
//
// muP_Iso_Ntracks_TTVA_custom
// muP_Iso_Ntracks_all_c5
// muP_Iso_Ntracks_all_c6
Int_t ColorIndex(0);
ColorIndex = 3;
TH1F* hRun1 = RetrieveHistRun1(DataType, Run1muPiso, ColorIndex, myPileup);
ColorIndex = 0;
// TH1F* h2016data_Default = RetrieveHist(theYear, DataType, Bisolation, ColorIndex, FileDate, myPileup);
TH1F* h_1516 = RetrieveHist(theYear, DataType, muPisolation, ColorIndex, FileDate, myPileup);
ColorIndex = 1;
TH1F* h_asRun1 = RetrieveHist(theYear, DataType, muP_iso_7_Chi2_5_noAbs_LooSiHi1Pt05_T0000000127, ColorIndex, FileDate, myPileup);
// ColorIndex = 4;
// TH1F* h_custom_approx = RetrieveHist(theYear, DataType, muP_iso_10_noChi2_LooSiHi1Pt05_T0004194304, ColorIndex, FileDate, myPileup);
ColorIndex = 2;
TH1F* h_customDerivation = RetrieveHist(theYear, DataType, muP_iso_10_noChi2_LooSiHi1Pt05_T0134217728, ColorIndex, FileDate, myPileup);
ColorIndex = 5;
TH1F* h_looseDerivation = RetrieveHist(theYear, DataType, muP_iso_10_noChi2_LooSiHi1Pt05_T0008388608, ColorIndex, FileDate, myPileup);
...
// 1)_10_noChi2_LooSiHi1Pt05_T0008388608            —-> BGJ (PPP)   
...
leg2->AddEntry(h_1516, "ACH (OOO)","pe");
leg2->AddEntry(h_asRun1, "BDI (PVP)","pe");
// leg2->AddEntry(h_custom_approx, "BFJ (PVP)","pe");
leg2->AddEntry(h_customDerivation, "BEJ (PPP)","pe");
leg2->AddEntry(h_looseDerivation, "BGJ (PPP)","pe");
}else if(DataType.EqualTo("mc")){
if (!myPileup.EqualTo("high")){
leg2->AddEntry(hRun1, "Run 1 Datasets","l");}
leg2->AddEntry(h_asRun1, "BDI (PVP)","l");
leg2->AddEntry(h_1516, "ACH (OOO)","l");
// leg2->AddEntry(h_custom_approx, "BFJ (PVP)","l");
leg2->AddEntry(h_customDerivation, "BEJ (PPP)","l");
leg2->AddEntry(h_looseDerivation, "BGJ (PPP)","l");
    }
// cout<<"DEBUG: 6"<<endl;
...
void muP_Iso_MultiVar_TTVA_v8_newVars(){
    TString FileDate = "";
    // FileDate = "2019July30";
    // FileDate = "2019Nov21";
    // FileDate = "2019Dec5";
    ...
    TString myPileup = "";
    myPileup = "all";
    makePlots("", "data", FileDate, myPileup);
    makePlots("", "mc", FileDate, myPileup);
