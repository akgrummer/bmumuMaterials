...
if (DataType.EqualTo("data")) {
    if (tmpYear.Contains("2016")){
        ...
        ifilename = DirName + "LASTskim.user.agrummer.Data1516.merged.root";
    }
    else{
    ...
    if (tmpYear.Contains("2016")){
        ...
        ifilename = DirName + "LASTskim.user.agrummer.mcBsmumu.merged.root";
    }else{
        // ifilename = DirName + tmp_mcName+"All/LASTskim_"+tmp_mcName+"_All_2019Jun3.root";
        ifilename = DirName + "LASTskim.user.agrummer.mcBsmumu.merged.root";
    }
        myCut= CutAdd*"PVWeights*Muon1_reco_eff_sf*Muon2_reco_eff_sf";
    }
...
ColorIndex = 0;
h_1516 = RetrieveHist(theYear, DataType, closeTrkNtrks_80, ColorIndex, FileDate, myPileup);
// ColorIndex = 4;
// h_customApprox = RetrieveHist(theYear, DataType, closeTrkNtrksChi2_LooSiHi1Pt05_corc2cnt_custom, ColorIndex, FileDate, myPileup);
ColorIndex = 2;
h_customTrkPerigee = RetrieveHist(theYear, DataType, closeTrkNtrksChi2_LooSiHi1Pt05_f2dc2_custom, ColorIndex, FileDate, myPileup);
// ColorIndex = 1;
...
// closeTrkNtrksChi2_LooSiHi1Pt05_corc2cnt
...
ColorIndex = 0;
h_1516 = RetrieveHist(theYear, DataType, closeTrkDOCA_3, ColorIndex, FileDate, myPileup);
// ColorIndex = 4;
// h_customApprox = RetrieveHist(theYear, DataType, closeTrkDOCA_LooSiHi1Pt05_corc2cnt_custom, ColorIndex, FileDate, myPileup);
ColorIndex = 2;
h_customTrkPerigee = RetrieveHist(theYear, DataType, closeTrkDOCA_T0134217728_LooSiHi1Pt05_f2dc2, ColorIndex, FileDate, myPileup);
// ColorIndex = 1;
// h_looseApprox = RetrieveHist(theYear, DataType, closeTrkDOCA_LooSiHi1Pt05_corc2cnt_loose, ColorIndex, FileDate, myPileup);
ColorIndex = 5;
h_looseTrkPerigee = RetrieveHist(theYear, DataType, closeTrkDOCA_T0008388608_LooSiHi1Pt05_f2dc2, ColorIndex, FileDate, myPileup);
// h_1516
// h_customApprox
// h_customTrkPerigee
// h_looseApprox
// h_looseTrkPerigee
// closeTrkDOCA_LooSiHi1Pt05_corc2cnt
...
TCanvas* c2 = new TCanvas("c2", "Var Comps",0.,0., 800, 600);
    ...
    if(DataType.EqualTo("data")){
        ...
        //don't use SetLimits for Histograms!!
            ...
            h_1516->GetXaxis()->SetRangeUser(xmin, xmax);// along X
...
if(DataType.EqualTo("data")){
    ...
    leg2->AddEntry(h_1516, "ACK (OOO)","pe");
    if (theVar.EqualTo("DOCA")){
        // leg2->AddEntry(h_customApprox, "BFM (PVV)","pe");
        leg2->AddEntry(h_customTrkPerigee, "BEL (PPP)","pe");
        // leg2->AddEntry(h_looseApprox, "BGM (PPV)","pe");
        leg2->AddEntry(h_looseTrkPerigee, "BGL (PPP)","pe");
    }else{
        // leg2->AddEntry(h_customApprox, "BFM (PVV)","pe");
        leg2->AddEntry(h_customTrkPerigee, "BEL (PPP)","pe");
        // leg2->AddEntry(h_looseApprox, "BGM (PPV)","pe");
    ...
    leg2->AddEntry(h_1516, "ACK (OOO)","l");
    if (theVar.EqualTo("DOCA")){
        // leg2->AddEntry(h_customApprox, "BFM (PVV)","l");
        leg2->AddEntry(h_customTrkPerigee, "BEL (PPP)","l");
        // leg2->AddEntry(h_looseApprox, "BGM (PPV)","l");
        leg2->AddEntry(h_looseTrkPerigee, "BGL (PPP)","l");
    }else{
        // leg2->AddEntry(h_customApprox, "BFM (PVV)","l");
        leg2->AddEntry(h_customTrkPerigee, "BEL (PPP)","l");
        // leg2->AddEntry(h_looseApprox, "BGM (PPV)","l");
...
c2->SaveAs(OutdirName + "VarComps2020May14_VarBinned_v8_newVars/"+varDir+"/"+typeDir+"/VarComp_"+Tag+".pdf");
delete c2;
...
void OtherIso_MultiVar_TTVA_v8_newVars() {
...
FileDate = "2020May14";
    ...
    myPileup = "all";
    makePlots("", "data", myVar, FileDate, myPileup);
    makePlots("", "mc", myVar, FileDate, myPileup);
