...
//script copied from PlotROCs_2020Jun26_PUtrained
// void MultiROC_wRun1(){
void PlotROCs_accept_errors_customTVA(){
    SetAtlasStyle();
...
TString idir4 = "/afs/cern.ch/user/a/agrummer/www/bmumu2020/BDT/BDToutput/2020Aug6/";
TString dirName1 = idir+"out_Custom/";
// TString dirName2 = idir2+"out_Run1privxmu/";
TString dirName2 = idir4+"Custom_privxmu/";
TString dirName3 = idir4+"Custom_nPV/";
...
myfilenames.push_back(dirName1 +"EvalROC_accept_error.root");
myfilenames.push_back(dirName2 +"EvalROC_accept_error.root");
myfilenames.push_back(dirName3 +"EvalROC_accept_error.root");
...
c2->SetLogy(1);
TGraphErrors *graph1 = (TGraphErrors*)ifilename1->Get("Graph");
graph1->SetLineColor(kGreen+2);//kBlack
// graph1->SetFillStyle(3214);
graph1->SetFillStyle(3002);
graph1->SetFillColorAlpha(kGreen+2, 0.4);//kBlack
...
TGraphErrors *graph2 = (TGraphErrors*)ifilename2->Get("Graph");
graph2->SetLineColor(kRed+1);//kBlack
// graph2->SetFillStyle(3241);
graph2->SetFillStyle(3002);
graph2->SetFillColorAlpha(kRed+1, 0.4);//kBlack
...
// TGraphErrors *graph3 = (TGraphErrors*)ifilename3->Get("Graph");
// graph3->SetLineColor(kBlue+1);//kBlack
// ->grROC->SetFi(1001);
// graph3->SetFillColorAlpha(kBlue+1, 0.4);//kBlack
...
// TGraphErrors *graph4 = (TGraphErrors*)ifilename4->Get("Graph");
// graph4->SetLineColor(kMagenta+2);//kBlack
// // graph4->SetFillStyle(3215);
// graph4->SetFillStyle(3002);
// graph4->SetFillColorAlpha(kMagenta+2, 0.4);//kBlack
...
// TGraphErrors *graph5 = (TGraphErrors*)ifilename5->Get("ROCDATA12BDT12_1_0");
TGraphErrors *graph5 = (TGraphErrors*)ifilename5->Get("Graph");
graph5->SetLineColor(kBlack);//kBlack
// graph5->SetFillStyle(3251);
graph5->SetFillStyle(3002);
graph5->SetFillColorAlpha(kBlack, 0.4);//kBlack
...
leg->AddEntry(graph1, "Custom TVA Iso Vars no PU" ,"l");
// leg->AddEntry(graph2, "With PRIVX mu Variable" ,"l");
leg->AddEntry(graph2, "Custom TVA Iso Vars + PRIVX mu (rounded)" ,"l");
leg->AddEntry(graph5, "Custom TVA Iso Vars + nPV (NPV weights)" ,"l");
// leg->AddEntry(graph4, "With nPV Variable" ,"l");
// leg->AddEntry(graph4, "Run 1 B Iso + Run 2 closeTrk + nPV (NPV weights)" ,"l");
...
// c2->SaveAs(idir3+"ROC_CustomPUtrained_2020Aug6.pdf");
c2->SaveAs(idir4+"ROC_CustomPUtrained_2020Aug6.pdf");
