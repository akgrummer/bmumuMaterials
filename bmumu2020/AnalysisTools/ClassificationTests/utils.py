...
MAX_NSCALE=1.4
#--------------------------------------------------------
#Produce JSON files with branch:index correspondence
#--------------------------------------------------------
def GetIndexes(chain, branch_name="event_number"):
    d_branch_index=dict()

    print("Making %s---> index dictionary." % branch_name)
    ...
    for i_entry, entry in enumerate(chain):
        ...
        d_branch_index[getattr(entry, branch_name)] = i_entry

    chain.SetBranchStatus("*", 1)

    return d_branch_index
#*******************************************************
...
def GetHistogram2D(name, title, nbins_x, min_x, max_x, nbins_y, min_y, max_y, title_x, title_y):
    ...
    return hist
#*********************************************************
def SaveHistograms(l_orig_histograms, out_path, kind , process, normalize=False, log=False, logx=False, legend=0, ratio=False):
    l_histograms=list()
