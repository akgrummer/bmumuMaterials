...
void cutBatchBsmumu(TString workdir="") {
  ...
  bool isMC(true);
  if ( !workdir.Contains("MC") ) isMC = false;
...
//  TString QLCfileName = "/afs/cern.ch/user/i/ibragimo/test/QLCfile_BsToMuMu.root";
  TString QLCfileName = "/eos/atlas/atlascerngroupdisk/phys-beauty/BsMuMuRun2/DDW/v6/QLCfile_BsToMuMu.root";
...
//  TriggerWeights myWeights("singleMu_SF_data1516_perperiod.root");
  ...
  if (dpdf != NULL){
      ...
      for (auto ievent=0; ievent<nevents; ievent++)
	{
      ...
      if (!tout->GetEntries()) {
        cout<< "No Bsmumu candidates, closing the files without writing, continuing. "<< endl;
        fout->cd();
        fout->Close();
        fin->cd();
        fin->Close();
