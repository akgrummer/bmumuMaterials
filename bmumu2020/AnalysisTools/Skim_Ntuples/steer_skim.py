...
def check():
    if len(args_steer) < 2:
        ...
        print("python steer_skim.py test Nevents DSID Process")
        print("python steer_skim.py DSID Process")
        exit(1)
...
PATH="/nfs/munch_3/home/ghneimat/AnalysisTools/Skim_Ntuples/Input/"
...
JOB_NAME="out_%s_%s_skimming" % (d_arg["DSID"], d_arg["PROCESS"])
...
for i_job in d_jobs: 
    l_call=["sbatch", SJ_NAME, LOG_DIR, ERR_DIR, "sing_split.job", OUTPUT_DIR, str(i_job), d_arg["EXECUTABLE"],JOB_NAME ] 
